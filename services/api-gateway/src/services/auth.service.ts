...
const callauthService = (options: any) => {
...
const breaker = new CircuitBreaker(callauthService, breakerOptions);

// Fallback for when the circuit is open
breaker.fallback(() => {
    return { 
        status: 503, 
        data: { success: false, message: "auth service temporarily unavailable" } 
    };
});
