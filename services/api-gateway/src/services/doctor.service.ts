...
const callDoctorService = async (options: any) => {
  return httpClient(options);
};

const breaker = new CircuitBreaker(callDoctorService, {
  timeout: 10000, 
...
breaker.fallback(() => {
  return { status: 503, data: { success: false, message: "Doctor service temporarily unavailable" } };
});
